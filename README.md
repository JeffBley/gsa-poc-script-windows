...
- Sets several registry keys which configure the following:
  ...
  - Unhides several buttons in the GSA client
  - Disables DNS over HTTPS in Edge and Chrome browsers (if corresponding browser is present)
  - Disables QUIC in Edge and Chrome browsers (if corresponding browser is present)
- Creates a policy.json file in the Firefox folder to disable DNS over HTTPS and QUIC if Firefox is present
- Prompts for device reboot if the script needed to set registry key for IPv4Preferred (change won't take effect until device reboot)
...
# Registry settings for GSA client, Edge DoH/QUIC, Chrome DoH/QUIC
$RegistrySettings = @(
    # GSA-related keys
...
# --- Check if GSA client already installed and install if not present ---
if (Test-Path $ExePath) {
    ...
    try {
        ...
        Write-Host "Could not retrieve version info. Please check $ReleaseHistoryUrl manually." -ForegroundColor Yellow
    }
} else {
...
# 1) Always apply NON-browser (GSA) keys exactly as before
